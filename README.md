...
* **XML Schema for DCAT-AP**: GeoNetwork is capable of storing metadata in XML format. The plugin therefore defines its own XML Schema (see the [schema](/src/main/plugin/metadata-dcat/schema) folder) for DCAT-AP that is used for the internal representation of DCAT-AP fields. To limit the data conversion needed, the XML Schema was designed to fully resemble an XML/RDF syntax of DCAT-AP.
...
* **validation (XSD and Schematron)**: Validation steps are first XSD validation made on the schema, then the schematron validation defined in folder  [metadata-dcat/schematron](/src/main/plugin/metadata-dcat/schematron). Two rule sets are available: schematron-rules-metadata-dcat, and schematron-rules-metadata-dcat-recommendations.
...
To include this schema plugin in a build, copy the metadata-dcat schema folder in the schemas folder, add it to the schemas/pom.xml and add it to the copy-schemas execution in web/pom.xml.

The best approach is to add the plugin as a submodule into GeoNetwork schema module.

```
cd schemas
git submodule add <metadata-dcat remote URL>
git submodule init
...
<dependency>
  ...
  <artifactId>schema-metadata-dcat</artifactId>
...
<execution>
  ...
  ...
  <resource>
    <directory>${project.basedir}/../schemas/metadata-dcat/src/main/plugin</directory>
...
```
cd ..   (core-geonetwork)
git am --ignore-space-change --ignore-whitespace --reject --whitespace=fix schemas/metadata-dcat/core-geonetwork-patches/*.patch
```
