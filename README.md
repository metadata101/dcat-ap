...
* **validation (XSD and Schematron)**: Validation steps are first XSD validation made on the schema, then the schematron validation defined in folder  [dcat-metadata/schematron](/src/main/plugin/dcat-metadata/schematron). Two rule sets are available: schematron-rules-dcat-metadata, and schematron-rules-dcat-metadata-recommendations.
