...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <!-- Surround the concept with parent based on  related parent xml element
    ...
    -->
  <xsl:template name="to-dcat-metadata-concept">
    <xsl:variable name="listOfLanguage" select="tokenize(/root/request/lang, ',')"/>
    <xsl:apply-templates mode="to-dcat-metadata-concept" select=".">
  ...
  <xsl:template mode="to-dcat-metadata-concept" match="*[not(/root/request/skipdescriptivekeywords)]">
  ...
  <xsl:template name="to-md-concept">
    ...
    <xsl:variable name="currentThesaurus"
                  select="if (thesaurus/key) then thesaurus/key else /root/request/thesaurus"/>
    <!-- Get  -->
    <xsl:variable name="inSchemeURI" select="gn-fn-dcat-metadata:getInSchemeURIByThesaurusId($currentThesaurus)"/>
