...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template mode="mode-dcat-metadata"
                match="gn:*|@gn:*|@*"
                priority="1000">
  </xsl:template>
  ...
  <xsl:template mode="mode-dcat-metadata" match="gn:child" priority="2000">
  ...
  <xsl:template mode="mode-dcat-metadata" priority="200"
    ...
    <xsl:call-template name="render-boxed-element">
      ...
      <xsl:with-param name="subTreeSnippet">
        ...
        <xsl:if test="$isEditing">
          ...
          <xsl:apply-templates mode="render-for-field-for-attribute-dcat-metadata"
                               select="@*|gn:attribute[not(@name = parent::node()/@*/name())]">
            <xsl:with-param name="ref" select="gn:element/@ref"/>
          </xsl:apply-templates>
        </xsl:if>
        <xsl:apply-templates mode="mode-dcat-metadata" select="*">
  ...
  <!--
    ...
    of the metadocument. Display in editing mode only and if
  the editor mode is not flat mode. -->
  <xsl:template mode="mode-dcat-metadata" match="gn:child[contains(@name, 'CHOICE_ELEMENT')]"
    priority="3000">
  ...
  <xsl:template mode="mode-dcat-metadata" priority="101"
  ...
  <xsl:template mode="mode-dcat-metadata" priority="100" match="dc:*|dct:*|dcat:*|vcard:*|foaf:*|spdx:*|adms:*|owl:*|schema:*|skos:*|mvs:*">
    <xsl:param name="schema" select="$schema" required="no"/>
    <xsl:param name="labels" select="$labels" required="no"/>
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        ...
        <xsl:call-template name="render-element">
          ...
          <xsl:with-param name="xpath" select="$xpath"/>
          <!--xsl:with-param name="forceDisplayAttributes" select="gn-fn-dcat-metadata:isForceDisplayAttributes(.)"/-->
  ...
  <xsl:template mode="render-for-field-for-attribute-dcat-metadata" match="@*[$isFlatMode and not(name(..)='dct:LicenseDocument')]|@gn:xsderror|@gn:addedObj" priority="101"/>

  <xsl:template mode="render-for-field-for-attribute-dcat-metadata" match="@*" priority="100">
  ...
  <xsl:template mode="render-for-field-for-attribute-dcat-metadata"
  ...
  <xsl:template mode="render-for-field-for-attribute-dcat-metadata"
    ...
    <xsl:if test="not($isFlatMode) or gn-fn-metadata:isFieldFlatModeException($viewConfig, $name, $xpath)">
      <xsl:if test="not(gn-fn-dcat-metadata:isNotMultilingualField(.., $editorConfig))">
