...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <!-- main template - the way into processing dcat-metadata -->
  <xsl:template name="metadata-dcat-metadata">
    <xsl:param name="schema"/>
    <xsl:param name="edit" select="false()"/>
    <xsl:param name="embedded"/>

    <xsl:apply-templates mode="dcat-metadata" select="." >
  ...
  <!-- simple -->
  <xsl:template name="metadata-dcat-metadataview-simple" match="metadata-dcat-metadataview-simple">
    ...
    <xsl:call-template name="md-content">
      ...
      <xsl:with-param name="relatedResources">
        <xsl:call-template name="dcat-metadata-relatedResources"/>
      </xsl:with-param>
      <xsl:with-param name="tabs">
        <xsl:call-template name="complexElementSimpleGui">
          ...
          <xsl:with-param name="content">
            <xsl:apply-templates mode="dcat-metadata" select=".">
              <xsl:with-param name="schema" select="'dcat-metadata'"/>
  ...
  -->
  <xsl:template name="dcat-metadata-relatedResources">
    <table class="related">
      <tbody>
  ...
  <!-- CompleteTab template - dc just calls completeTab from
       metadata-utils.xsl -->
  <xsl:template name="dcat-metadataCompleteTab">
  ...
  -->
  <xsl:template mode="dcat-metadata" match="*|@*">
    <xsl:param name="schema"/>
    <xsl:param name="edit"/>
  ...
  -->
  <xsl:template mode="dcat-metadata" match="dcat:Dataset|csw:Record">
    <xsl:param name="schema"/>
    <xsl:param name="edit"/>
  ...
  <xsl:template mode="dcat-metadata" match="dct:anyCHOICE_ELEMENT0">
    <xsl:param name="schema"/>
    <xsl:param name="edit"/>
  ...
  identifier
  -->
  <xsl:template mode="dcat-metadata" match="dct:identifier">
    <xsl:param name="schema"/>
    <xsl:param name="edit"/>
  ...
  <xsl:template mode="dcat-metadata" match="dct:description|dct:rights|dct:title">
    <xsl:param name="schema"/>
    <xsl:param name="edit"/>
  ...
  <xsl:template mode="dcat-metadata" match="dct:date|dct:created|dct:dateSubmitted|
  ...
  <xsl:template mode="dcat-metadata" match="dct:spatial">
    <xsl:param name="schema"/>
    <xsl:param name="edit"/>
  ...
  <!-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -->
  <!-- dcat-metadata brief and superBrief formatting -->
  <!-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -->
  ...
  <xsl:template name="dcat-metadataBrief">
    <metadata>
      <xsl:if test="dct:title">
        <title><xsl:value-of select="dct:title"/></title>
      </xsl:if>
