...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:function name="gn-fn-metadata-dcat:getInSchemeURIByElementName" as="xs:string">
    ...
    <xsl:choose>
      ...
      <xsl:when test="$elementName = 'mvs:statusVanGebruik'">
        <xsl:value-of select="concat($inSchemeMvsBaseUrl,'gebruiksstatus')"/>
      </xsl:when>
      <xsl:when test="$elementName = 'mvs:statusVanOntwikkeling'">
        <xsl:value-of select="concat($inSchemeMvsBaseUrl,'ontwikkelingsstatus')"/>
      </xsl:when>
      <xsl:otherwise>
        <xsl:value-of select="''"/>
      </xsl:otherwise>
  ...
  <xsl:function name="gn-fn-metadata-dcat:getRdfTypeByElementName" as="xs:string">
    <xsl:param name="elementName"/>
    <xsl:param name="parentElementName"/>
    <xsl:choose>
      <xsl:when test="$elementName = 'dct:type' and $parentElementName = 'foaf:Agent'">
  ...
  <xsl:function name="gn-fn-metadata-dcat:getThesaurusTitle" as="xs:string">
    ...
    <xsl:choose>
      ...
      <xsl:when test="$resource = concat($inSchemeMvsBaseUrl,'gebruiksstatus')">
        <xsl:value-of select="'Conceptscheme gebruiksstatus'"/>
      </xsl:when>
      <xsl:when test="$resource = concat($inSchemeMvsBaseUrl,'ontwikkelingsstatus')">
        <xsl:value-of select="'Conceptscheme ontwikkelingsstatus'"/>
      </xsl:when>
      <xsl:otherwise>
  ...
  <xsl:function name="gn-fn-metadata-dcat:getThesaurusIdentifier" as="xs:string">
    <xsl:param name="resource"/>
    <xsl:choose>
      <xsl:when test="$resource = concat($inSchemeAdmsBaseUrl,'publishertype/1.0')">
      ...
      <xsl:when test="$resource = concat($inSchemeMvsBaseUrl,'gebruiksstatus')">
        <xsl:value-of select="concat($thesaurusIdentifierBaseKey,'gebruiksstatus')"/>
      </xsl:when>
      <xsl:when test="$resource = concat($inSchemeMvsBaseUrl,'ontwikkelingsstatus')">
        <xsl:value-of select="concat($thesaurusIdentifierBaseKey,'ontwikkelingsstatus')"/>
      </xsl:when>
      <xsl:otherwise>
  ...
  <xsl:function name="gn-fn-metadata-dcat:isNotMultilingualField" as="xs:boolean">
  ...
  -->
  <xsl:function name="gn-fn-metadata-dcat:getField" as="node()">
  ...
  <xsl:function name="gn-fn-metadata-dcat:isForceDisplayAttributes" as="xs:boolean">
    <xsl:param name="element" as="node()"/>
    <xsl:choose>
  ...
  <xsl:function name="gn-fn-metadata-dcat:getBboxCoordinates" as="xs:string">
  ...
  <xsl:function name="gn-fn-metadata-dcat:concatXPaths" as="xs:string">
