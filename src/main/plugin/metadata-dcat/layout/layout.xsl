...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template mode="mode-metadata-dcat"
                match="gn:*|@gn:*|@*"
                priority="1000">
  </xsl:template>
  ...
  <xsl:template mode="mode-metadata-dcat" match="gn:child" priority="2000">
  ...
  <xsl:template mode="mode-metadata-dcat" priority="200"
  ...
  <!--
    ...
    of the metadocument. Display in editing mode only and if
  the editor mode is not flat mode. -->
  <xsl:template mode="mode-metadata-dcat" match="gn:child[contains(@name, 'CHOICE_ELEMENT')]"
    priority="3000">
  ...
  <xsl:template mode="mode-metadata-dcat" priority="101"
  ...
  <xsl:template mode="mode-metadata-dcat" priority="100" match="dc:*|dct:*|dcat:*|vcard:*|foaf:*|spdx:*|adms:*|owl:*|schema:*|skos:*|mdcat:*">
    <xsl:param name="schema" select="$schema" required="no"/>
    <xsl:param name="labels" select="$labels" required="no"/>
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        ...
        <xsl:call-template name="render-element">
          ...
          <xsl:with-param name="xpath" select="$xpath"/>
          <!--xsl:with-param name="forceDisplayAttributes" select="gn-fn-metadata-dcat:isForceDisplayAttributes(.)"/-->
  ...
  <xsl:template mode="mode-metadata-dcat" priority="2001"
  ...
  <xsl:template mode="render-for-field-for-attribute-metadata-dcat" match="@*" priority="100">
  ...
  <xsl:template mode="render-for-field-for-attribute-metadata-dcat"
  ...
  <xsl:template mode="render-for-field-for-attribute-metadata-dcat"
    ...
    <xsl:if test="not($isFlatMode) or gn-fn-metadata:isFieldFlatModeException($viewConfig, $name, $xpath)">
      <xsl:if test="not(gn-fn-metadata-dcat:isNotMultilingualField(.., $editorConfig))">
