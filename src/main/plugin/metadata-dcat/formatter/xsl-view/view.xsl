...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template mode="render-view" match="field[template]"
    ...
    <!-- Get all elements that are within a metadata-dcat namespace -->
    <xsl:variable name="elements">
      <xsl:call-template name="evaluate-metadata-dcat">
        <xsl:with-param name="base" select="$base" />
        <xsl:with-param name="in" select="concat('/../', $fieldXpath)" />
      </xsl:call-template>
    </xsl:variable>

    <!-- Render fields for each metadata-dcat element -->
  ...
  <xsl:template mode="render-field"
    ...
    <xsl:if test="normalize-space($stringValue) != '' and
      ...
      <tr>
        <th style="{$thStyle}">
          <xsl:value-of select="gn-fn-metadata:getLabel($schema, name(.), $labels, name(..), '', gn-fn-metadata-dcat:concatXPaths($xpath, gn-fn-metadata:getXPath(.), name(.)))/label" />
  ...
  <xsl:template mode="render-field"
    ...
    <xsl:if test="normalize-space($stringValue) != ''">
      <tr>
        <th style="{$thStyle}">
          <xsl:value-of select="gn-fn-metadata:getLabel($schema, 'rdf:resource', $labels, name(..), '', concat(gn-fn-metadata-dcat:concatXPaths($xpath, gn-fn-metadata:getXPath(.), name(.)), '/@rdf:resource'))/label" />
        </th>
        <td style="{$tdStyle}">
  ...
  <xsl:template mode="render-field" match="dct:Location">
    ...
    <xsl:variable name="bbox" select="gn-fn-metadata-dcat:getBboxCoordinates($geometry)"/>
