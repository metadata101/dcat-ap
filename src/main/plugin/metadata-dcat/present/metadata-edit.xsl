...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <!-- main template - the way into processing metadata-dcat -->
  <xsl:template name="metadata-metadata-dcat">
    <xsl:param name="schema"/>
    <xsl:param name="edit" select="false()"/>
    <xsl:param name="embedded"/>

    <xsl:apply-templates mode="metadata-dcat" select="." >
  ...
  <xsl:template name="metadata-metadata-dcatview-simple" match="metadata-metadata-dcatview-simple">
    ...
    <xsl:call-template name="md-content">
      ...
      <xsl:with-param name="relatedResources">
        <xsl:call-template name="metadata-dcat-relatedResources"/>
      </xsl:with-param>
      <xsl:with-param name="tabs">
        <xsl:call-template name="complexElementSimpleGui">
          ...
          <xsl:with-param name="content">
            <xsl:apply-templates mode="metadata-dcat" select=".">
              <xsl:with-param name="schema" select="'metadata-dcat'"/>
  ...
  -->
  <xsl:template name="metadata-dcat-relatedResources">
    <table class="related">
      <tbody>
  ...
  -->
  <xsl:template mode="metadata-dcat" match="*|@*">
    <xsl:param name="schema"/>
    <xsl:param name="edit"/>
  ...
  -->
  <xsl:template mode="metadata-dcat" match="dcat:Dataset|csw:Record">
    <xsl:param name="schema"/>
    <xsl:param name="edit"/>
  ...
  <xsl:template mode="metadata-dcat" match="dct:anyCHOICE_ELEMENT0">
    <xsl:param name="schema"/>
    <xsl:param name="edit"/>
  ...
  identifier
  -->
  <xsl:template mode="metadata-dcat" match="dct:identifier">
    <xsl:param name="schema"/>
    <xsl:param name="edit"/>
  ...
  <xsl:template mode="metadata-dcat" match="dct:description|dct:rights|dct:title">
    <xsl:param name="schema"/>
    <xsl:param name="edit"/>
  ...
  <xsl:template mode="metadata-dcat" match="dct:date|dct:created|dct:dateSubmitted|
  ...
  <xsl:template mode="metadata-dcat" match="dct:spatial">
    <xsl:param name="schema"/>
    <xsl:param name="edit"/>
  ...
  <!-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -->
  <!-- metadata-dcat brief and superBrief formatting -->
  <!-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -->
