...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <!-- Surround the concept with parent based on  related parent xml element
    ...
    -->
  <xsl:template name="to-metadata-dcat-concept">
    <xsl:variable name="listOfLanguage" select="tokenize(/root/request/lang, ',')"/>
    <xsl:apply-templates mode="to-metadata-dcat-concept" select=".">
  ...
  <xsl:template mode="to-metadata-dcat-concept" match="*[not(/root/request/skipdescriptivekeywords)]">
    ...
    <xsl:choose>
      ...
      <xsl:when test="ends-with($thesaurusKey, 'levensfase')">
        <mdcat:levensfase>
          <xsl:copy-of select="$concept"/>
        </mdcat:levensfase>
      </xsl:when>
      <xsl:when test="ends-with($thesaurusKey, 'ontwikkelingstoestand')">
  ...
  <xsl:template name="to-md-concept">
    ...
    <!-- Get  -->
    <xsl:variable name="inSchemeURI" select="gn-fn-metadata-dcat:getInSchemeURIByThesaurusId($currentThesaurus)"/>
